...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isProduction && !hasInngestKeys) {
    ...
    if (!isProduction && !hasInngestKeys) {
      console.warn('[JOBS_SCRAPE] ⚠️ Inngest keys not set. For local development, run: npx inngest-cli@latest dev');
    }
    ...
    try {
      ...
      console.error('[JOBS_SCRAPE] Failed to send Inngest event:', sendError);
      return NextResponse.json(
        {
          success: false,
