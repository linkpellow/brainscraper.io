/**
 * Test if Cognito Access Token works directly with USHA DNC API
 */

import { getCognitoIdToken, getCognitoAccessToken } from './utils/cognitoAuth';

async function testCognitoAccessTokenWithDNC() {
  console.log('🧪 Testing Cognito Access Token with USHA DNC API...\n');
  ...
  try {
    ...
    console.log('\n❌ Neither Cognito token works directly with USHA DNC API');
...
// Run the test
testCognitoAccessTokenWithDNC()
  .then(result => {
    if (result?.success) {
      console.log(`\n✅ SOLUTION: Use Cognito ${result.tokenType} directly!`);
      console.log('   No token exchange needed.');
    } else {
      console.log('\n⚠️  Token exchange is still required.');
    }
